...
public class UpdateLocation implements Runnable {
    ...
    public UpdateLocation(LocationManager locationManager, Context context, Controller controller) {
        ...
        this.locationManager.requestLocationUpdates(LocationManager.NETWORK_PROVIDER, 1000, 0, ll);
        this.locationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, 1000, 0, ll);
    }
    ...
    public void run() {
        while (run) {
            buffer.get();
            List<String> ids = controller.getIds();
            if (ids != null) {
                if (ids.size() > 0) {
